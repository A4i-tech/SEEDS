...
describe('WebSocketService', () => {
    ...
    beforeEach(() => {
        ...
        azureBlobService.getBlobData.mockResolvedValue(Buffer.from('mock audio data'));
    ...
    describe('playAudioContent', () => {
        ...
        it('should handle successful playback and state management', async () => {
            ...
            expect(connectionManager.getConnection).toHaveBeenCalledWith('test-client');
            expect(azureBlobService.getBlobData).toHaveBeenCalledWith('container', 'audio.wav');
        ...
        it('should handle errors and URL parsing', async () => {
            ...
            await expect(websocketService.playAudioContent('non-existent', testBlobUrl))
            ...
            await expect(websocketService.playAudioContent('test-client', testBlobUrl))
                .rejects.toThrow('WebSocket is not open');
            mockWebSocket.readyState = 1;

            // Test nested folder URL parsing
            const nestedUrl = 'https://storage.example.com/container/folder/subfolder/audio.wav';
            await websocketService.playAudioContent('test-client', nestedUrl);
            expect(azureBlobService.getBlobData).toHaveBeenCalledWith('container', 'folder/subfolder/audio.wav');
        });
    });

    describe('playSystemAudioContent', () => {
        const testBlobUrl = 'https://storage.example.com/container/system-audio.wav';
        ...
        it('should handle system audio queuing and state management', async () => {
            ...
            mockState.audioContentState = { playing: true, blobUrl: 'test-url', position: 50 };
            ...
            expect(mockControlWebSocket.send).toHaveBeenCalledWith(
                JSON.stringify({ websocket_id: 'test-client', type: 'playback-state-update', message: PlaybackStatus.PAUSED }),
                expect.any(Function)
            );

            // Test multiple system audio queuing (reset to queuing state)
            mockState.systemAudioContentQueue = [{ blobUrl: testBlobUrl }];
            const secondUrl = 'https://storage.example.com/container/system-audio-2.wav';
    ...
    describe('control methods', () => {
        it('should handle pause, resume, stop, close, and reconnection', () => {
            ...
            // Test error cases
            connectionManager.getConnection.mockReturnValue(undefined);
            expect(() => websocketService.pauseAudioContent('non-existent')).toThrow('WebSocket connection not found');

            // Reset connection and test resume error
            connectionManager.getConnection.mockReturnValue(mockConnection);
            mockState.audioContentState = null;
            expect(() => websocketService.resumeAudioContent('test-client')).toThrow('No audio content data to resume');
    ...
    describe('streaming and error handling', () => {
        it('should handle audio streaming with timing and error scenarios', async () => {
            ...
            await websocketService.playAudioContent('test-client', 'https://storage.example.com/container/test.wav');
            ...
            await websocketService.playAudioContent('test-client', 'https://storage.example.com/container/test.wav');
            jest.advanceTimersByTime(50);
            expect(consoleSpy).toHaveBeenCalledWith('Error sending data over WebSocket for ID: test-client', testError);
            expect(mockWebSocket.close).toHaveBeenCalled();
        ...
        it('should handle edge cases and service errors', async () => {
            ...
            await expect(websocketService.playAudioContent('test-client', 'https://storage.example.com/container/test.wav'))
                .rejects.toThrow('Blob service error');
            azureBlobService.getBlobData.mockResolvedValue(Buffer.from('mock audio data'));

            // Test control WebSocket errors
            const controlError = new Error('Control send error');
            mockControlWebSocket.send.mockImplementation((data, callback) => callback(controlError));
            mockState.currentAudioType = 'audioContent';
            mockState.audioContentState = { playing: true };
            websocketService.pauseAudioContent('test-client');
            expect(consoleSpy).toHaveBeenCalledWith('Error sending data over WebSocket for ID: test-client', controlError);
            mockControlWebSocket.send.mockImplementation((data, callback) => callback && callback());
