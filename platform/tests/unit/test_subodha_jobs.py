...
async def test_create_job_generates_id_and_persists(repo):
    job = await subodha_jobs.create_job(repo, tenant_id="tenant-a", scope="all", course_id=None, total_courses=0)
    assert job["_id"]
    stored = await repo.get_job("tenant-a", job["_id"])
...
async def test_subscribe_replays_done_immediately_for_finished_job(repo):
    job = await subodha_jobs.create_job(repo, tenant_id="tenant-a", scope="all", course_id=None, total_courses=1)
    await subodha_jobs.finish_job(repo, "tenant-a", job["_id"], "completed")

    events = []
    async for event in subodha_jobs.subscribe(repo, "tenant-a", job["_id"]):
        events.append(event)
...
async def test_subscribe_streams_live_progress_then_done(repo):
    ...
    await subodha_jobs.record_course_result(
        repo, "tenant-a", job["_id"], {"courseId": "c1", "name": "Course One", "status": "saved", "error": None, "at": "now"}
    )
    await subodha_jobs.finish_job(repo, "tenant-a", job["_id"], "completed")
...
async def test_subscribe_unknown_job_yields_nothing(repo):
    events = [e async for e in subodha_jobs.subscribe(repo, "tenant-a", "no-such-job")]
    assert events == []
...
async def test_subscribe_wrong_tenant_yields_nothing(repo):
    job = await subodha_jobs.create_job(repo, tenant_id="tenant-a", scope="all", course_id=None, total_courses=1)
    events = [e async for e in subodha_jobs.subscribe(repo, "tenant-b", job["_id"])]
    assert events == []
