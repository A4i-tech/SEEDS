#!/usr/bin/env python3
"""
Migration 003 — Refresh-token collection indexes.
...
token_id lookup (try_consume / find_by_token_id) and the owner-scoped
revoke_family / revoke_all_for_owner sweeps. Idempotent — running twice
will not create duplicate indexes.

Usage:
    python migrations/003_refresh_token_indexes.py [--dry-run] [--mongo-uri URI]
...
async def migrate(mongo_uri: str, dry_run: bool) -> None:
    """Create all refresh-token collection indexes."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add refresh-token collection indexes (userRefreshTokens, integrationTokens)."
    )
