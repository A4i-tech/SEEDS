...
from app.models.responses.common import ConferenceStatusResponse, EventQueuedResponse, MessageResponse
...
async def create_start_conference(request: CreateConferenceRequest) -> ConferenceStatusResponse:
    conf = await _create_conf(request)
    await conf.start_conference_call()
    return ConferenceStatusResponse(status="STARTED", id=conf.conf_id)
