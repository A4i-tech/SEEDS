...
def _resolve_language(
    ...
    if not detected or detected.lower() in ("auto", "detecting", "unknown"):
        logger.warning(
            "remediation: no detected language for job_id=%s; falling back to the source filename", job.job_id
        )
        detected = language_from_filename(job.source_name) or "English"
