...
def build_snippet(translation_sdk_base_url: str, base_url: str, site_id: str) -> str:
    return (
        "<script\n"
        f'  src="{translation_sdk_base_url}/sdk.js"\n'
...
class OnboardingService:
    ...
    async def register_website(
        ...
        self._translation_sdk_base_url()
        self._base_url()
    ...
    def _translation_sdk_base_url(self) -> str:
        translation_sdk_base_url = get_settings().translation_sdk_base_url.rstrip("/")
        if not translation_sdk_base_url:
            raise ConfigurationError("TRANSLATION_SDK_BASE_URL must be set to generate an SDK snippet")
        return translation_sdk_base_url
    ...
    def _snippet_for(self, website: dict[str, Any]) -> str:
        return build_snippet(self._translation_sdk_base_url(), self._base_url(), website["site_id"])
