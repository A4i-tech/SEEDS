...
class UnmuteAllEvent(ConferenceEvent):
    ...
    async def execute_event(self):
        ...
        if unmute_tasks:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    logger_instance.error(f"Failed to unmute student {student_phone}: {result}")
        ...
        logger_instance.info(f"UNMUTE ALL completed: {unmuted_count} students unmuted", self.conf_call.conf_id)
    
    async def _unmute_student(self, phone_number: str) -> bool:
        """Helper method to unmute a single student."""
        try:
            participant = self.conf_call.state.participants.get(phone_number)
            if not participant:
                return False
            
            # Unmute the participant via communication API
            await self.conf_call.communication_api.unmute_participant(phone_number)
            
            # Update participant's mute status
            participant.is_muted = False
            
            # Set raised hand to false when unmuting
