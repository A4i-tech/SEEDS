...
# pullMenuMainUrl = ""
pullMenuMainUrl = storage_account_base_url
content_url = f"https://{storage_account_name}.blob.core.windows.net/output-container/"
url = f'https://{storage_account_name}.azurewebsites.net/content'
headers = {
    'authToken': 'postman'
}
...
quiz_new = {
    ...
    "title": "Punyakoti",
    "titleAudio": f"{storage_account_base_url}experience-titles/quiz/12f77743-4255-48a8-855b-2f4d7b635c95/1.0.mp3",
    "localTitle": "ಮೊಲದ ಮರಿ",
    "positiveMarks": 1,
    "negativeMarks": 0,
    "type": "quiz",
    "questions": [
        {
            "question": {
                ...
                "url": f"{content_url}Quiz/Punyakoti/question_1/1.0.mp3",
                "text": "ಪುಣ್ಯಕೋಟಿಗೂ ತನ್ನ ಕರುವಿಗೂ ಏನು ಸಂಬಂಧ? "
            },
            "options": [
                {
                    "id": "3ef0ae75-9f9e-4d9a-8bfa-ac67297d15b0",
                    "url": f"{content_url}Quiz/Punyakoti/option_1/1/1.0.mp3",
                    "text": " ತಾಯಿ"
                },
                {
                    "id": "614c161e-a0b0-426f-913c-fc8adba39f8b",
                    "url": f"{content_url}Quiz/Punyakoti/option_1/2/1.0.mp3",
                    "text": " ಒಡಹುಟ್ಟಿದವರು"
                },
                {
                    "id": "f34e920d-78a5-4230-9bd0-d603acc941d5",
                    "url": f"{content_url}Quiz/Punyakoti/option_1/3/1.0.mp3",
                    "text": " ಸ್ನೇಹಿತ"
                },
                {
                    "id": "e8ba0f72-bf17-47b2-ba4b-6bd53615604a",
                    "url": f"{content_url}Quiz/Punyakoti/option_1/4/1.0.mp3",
                    "text": " ಚಿಕ್ಕಮ್ಮ "
                }
            ],
        ...
        },
        {
            "question": {
                ...
                "url": f"{content_url}Quiz/Punyakoti/question_2/1.0.mp3",
                "text": "ಪುಣ್ಯಕೋಟಿ ತನ್ನ ಸ್ನೇಹಿತರೊಂದಿಗೆ ಹುಲ್ಲು ತಿನ್ನುತ್ತಿರುವಾಗ ಯಾವ ಪ್ರಾಣಿ ತಡೆಯುತ್ತದೆ? "
            },
            "options": [
                {
                    "id": "a66288f0-f09d-4510-b747-26660b6dac8d",
                    "url": f"{content_url}Quiz/Punyakoti/option_2/1/1.0.mp3",
                    "text": " ಹುಲಿ "
                },
                {
                    "id": "d79f2970-e8a6-4b19-95a4-15c49db89886",
                    "url": f"{content_url}Quiz/Punyakoti/option_2/2/1.0.mp3",
                    "text": " ಸಿಂಹ"
                },
                {
                    "id": "1e4756b0-684b-48d0-9168-b544b3ad2396",
                    "url": f"{content_url}Quiz/Punyakoti/option_2/3/1.0.mp3",
                    "text": " ಬೇಟೆಗಾರ"
                },
                {
                    "id": "2f1a696e-7857-43fe-973f-50b599224946",
                    "url": f"{content_url}Quiz/Punyakoti/option_2/4/1.0.mp3",
                    "text": " ತೋಳ"
                }
            ],
        ...
        },
        {
            "question": {
                ...
                "url": f"{content_url}Quiz/Punyakoti/question_5/1.0.mp3",
                "text": "ಸತ್ಯದ ಬಗ್ಗೆ ಪುಣ್ಯಕೋಟಿ ಹೇಳಿದ್ದೇನು? "
            },
            "options": [
                {
                    "id": "5a72b87a-f7fe-4e19-9099-dabe763e6fc8",
                    "url": f"{content_url}Quiz/Punyakoti/option_5/1/1.0.mp3",
                    "text": " ಇದು ನಮ್ಮ ತಾಯಿ ಮತ್ತು ತಂದೆ"
                },
                {
                    "id": "79df7b0e-6ec2-4882-913f-33f1cac8f9d9",
                    "url": f"{content_url}Quiz/Punyakoti/option_5/2/1.0.mp3",
                    "text": " ಇದು ಮುಖ್ಯವಲ್ಲ"
                },
                {
                    "id": "0600e6e2-1055-4702-8396-8a331db133f3",
                    "url": f"{content_url}Quiz/Punyakoti/option_5/3/1.0.mp3",
                    "text": " ಇದು ಮೂರ್ಖರಿಗೆ"
                },
                {
                    "id": "3b27625b-9b25-442a-9adf-1727dd77165d",
                    "url": f"{content_url}Quiz/Punyakoti/option_5/4/1.0.mp3",
...
def getStreamActions(items_list, values_to_urls, level, state, parent_selections={}):
    ...
    if level == 0 and state == 0:
        actions.append(StreamAction(
            pullMenuMainUrl +'welcomeDialog/kannada/welcome%20to%20SEEDS/1.0.mp3'))
    complete_url = ''
    language = ''
    ...
    if state == 0:
        if category == 'theme':
            ...
            print("THEME URL replaced",
                  readingContentTitlesDialogUrl["theme"].replace('{language}', language).replace('{speechRate}',
                                                                                                 speechRate))
            print("LANGAUGE", language)

            actions.append(StreamAction(
                pullMenuMainUrl + readingContentTitlesDialogUrl["theme"].replace('{language}', language).replace(
                    '{speechRate}', speechRate)))
        elif category == 'title':
            actions.append(StreamAction(
                pullMenuMainUrl + readingContentTitlesDialogUrl[parent_selections['type'].lower()].replace('{language}',
                                                                                                           language).replace(
                    '{speechRate}', speechRate)))

    # Loop over the relevant items based on the current state and category
    for index, value_used_to_create_url in enumerate(items_list[state * number_of_categories_listed_in_one_state: min(
    ...
    if state != number_of_states_in_same_level - 1 and number_of_states_in_same_level > 1:
        ...
        key_to_value_mapping[int(next_n_categories_key)] = 'next ' + str(
            number_of_categories_listed_in_one_state) + ' items'
        # replaced_url = pressKeyMessageUrl.replace('{language}', language).replace('{speechRate}', str(speechRate))
        ...
        key_to_value_mapping[int(previous_n_categories_key)] = 'previous ' + str(
            number_of_categories_listed_in_one_state) + ' items'
    ...
    if level != 0 and len(content_attributes) > 1:
        previousMenuMessageUrl = goToPreviousMenuMessageUrl.replace('{language}', language).replace('{speechRate}',
                                                                                                    speechRate)
...
def generate_states(fsm, content_list, content_attributes, level, parent_state_id='', parent_selections={}):
    if level == len(content_attributes):
        ...
        if (parent_selections['type'] == 'quiz'):
            ...
            quiz.generate_states(fsm=fsm, prefix_state_id=parent_state_id[:-1],
                                 parent_block_state_id=parent_block_state_id, key_chosen=key_for_option_chosen,
                                 level=level)
        ...
        audioGoingTobePlayedUrl = audioGoingTobePlayedDialogUrl.replace('{language}', language).replace('{speechRate}',
                                                                                                        speechRate)
        ...
        fsm.add_transition(
            Transition(source_state_id=parent_block_state_id, dest_state_id=state_id, input=str(key_for_option_chosen),
                       actions=[]))

        fsm.add_transition(
            Transition(source_state_id=state_id, dest_state_id=parent_block_state_id, input=previous_category_level_key,
                       actions=[]))
        fsm.add_transition(
            Transition(source_state_id=state_id, dest_state_id=state_id, input=str(repeat_current_categories_key),
                       actions=[]))
        ...
        fsm.add_transition(
            Transition(source_state_id=state_id, dest_state_id=state_id_final_state, input="empty", actions=[]))

        fsm.add_transition(
            Transition(source_state_id=state_id_final_state, dest_state_id=fsm.end_state.id, input="empty", actions=[]))
        fsm.add_transition(Transition(source_state_id=state_id_final_state, dest_state_id=parent_block_state_id,
                                      input=previous_category_level_key, actions=[]))
        fsm.add_transition(Transition(source_state_id=state_id_final_state, dest_state_id=state_id,
                                      input=str(repeat_current_categories_key), actions=[]))
    ...
    if category == "language":
        ...
        for title in titles:
            if parent_selections['type'] == 'quiz':
                ...
                titleAudio = [x for x in filtered_content if x['title'] == title][0][
                                 'titleAudio'] + f'/{speechRate}.mp3'
                sorted_categories.append(titleAudio)
    ...
    for state_index in range(number_of_states_in_same_level):
        ...
        if level > 0 and state_index == 0:  # Add transition to the parent state_index
            ...
            indexOfDigit = option_chosen.find('(')
            key_for_option_chosen = int(option_chosen[0:indexOfDigit]) + 1
            fsm.add_transition(Transition(source_state_id=parent_block_state_id, dest_state_id=state_id,
                                          input=str(key_for_option_chosen), actions=[]))

    for state_index in range(number_of_states_in_same_level):
        state_id = f"{parent_state_id}{category_id_prefix}{state_index}"
        if state_index != 0 and number_of_states_in_same_level > 1:
            fsm.add_transition(Transition(source_state_id=state_id,
                                          dest_state_id=f"{parent_state_id}{category_id_prefix}{state_index - 1}",
                                          input=previous_n_categories_key, actions=[]))
        if state_index != number_of_states_in_same_level - 1 and number_of_states_in_same_level > 1:
            fsm.add_transition(Transition(source_state_id=state_id,
                                          dest_state_id=f"{parent_state_id}{category_id_prefix}{state_index + 1}",
                                          input=next_n_categories_key, actions=[]))
        fsm.add_transition(
            Transition(source_state_id=state_id, dest_state_id=state_id, input=repeat_current_categories_key,
                       actions=[]))
        if level != 0 and len(content_attributes) > 1:
            indexOfLastOp = parent_state_id.rfind('Op')
            parent_block_state_id = parent_state_id[:(indexOfLastOp - 1)]
            fsm.add_transition(Transition(source_state_id=state_id, dest_state_id=parent_block_state_id,
                                          input=previous_category_level_key, actions=[]))

        indexes_possible = min((state_index + 1) * number_of_categories_listed_in_one_state,
                               len(sorted_categories)) - state_index * number_of_categories_listed_in_one_state
...
async def instantiate_from_latest_content(content_ids: Optional[List[str]] = None):
    ...
    fsm.set_end_state(
        State(state_id="END", actions=[TalkAction(text="You didn't choose a valid option. Bye bye.", bargeIn=False)]))
    parent_selections = {}
