...
class IVRCallStateMongoDoc(BaseModel):
    ...
    tenant_id: str = ""  # Optional tenant ID field
...
class EventWebhookRequest(BaseModel):
    ...
    network: str | None = ""
    duration: str | None = (
        ""  # This is represented as a string, but you might consider converting it to an integer or float if it represents time in seconds or similar
    )
    start_time: str | None = ""
    rate: str | None = ""
    price: str | None = ""
    from_: str = Field(
        ..., alias="from"
    )  # 'from' is a Python reserved keyword, so we use an alias
    headers: dict = {}
    uuid: str
    to: str
    conversation_uuid: str
