...
from app.utils.model_classes import IVRCallStateMongoDoc, UserAction, VonageCallStartResponse
...
calls_log_mongo = MongoDB(db_name="ivr", collection_name="callLogs")
...
class CallProcessor:
    """
    Worker that processes IVR call messages from the Service Bus.
    """
    ...
    def update_fsm(self, fsm_dict: Dict, latest_id: str):
        ...
        logger.info(f"CallProcessor FSM updated to ID: {latest_id}")
    ...
    async def process_call_webhook(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"[CALL_WEBHOOK] Processing call webhook for phone number: {phone_number}, call_log_id: {call_log_id}")
            logger.info(f"[CALL_WEBHOOK] Message data: {message_data}")

            logger.info(f"[CALL_WEBHOOK] Calling _start_ivr_internal for {phone_number}...")
            start_ivr_response = await self._start_ivr_internal(phone_number)
            logger.info(f"[CALL_WEBHOOK] _start_ivr_internal response: {start_ivr_response}")
            
            if start_ivr_response.get("status_code") == 200:
                logger.info(f"[CALL_WEBHOOK] ✓ IVR started successfully for phone number: {phone_number}")
                ...
                logger.info(f"[CALL_WEBHOOK] ✓ Call log updated for call_log_id: {call_log_id} to 'called' status.")
            else:
                logger.warning(f"[CALL_WEBHOOK] ✗ IVR failed to start: {start_ivr_response}")
        except Exception as e:
            logger.error(f"[CALL_WEBHOOK] ✗ Error processing call webhook message: {e}", exc_info=True)
    ...
    async def process_dtmf_input(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Processing DTMF input for conversation_uuid: {conv_id}, digits: {digits}")

            # Get IVR state

            doc = await ongoing_fsm_mongo.find_by_id(conv_id)
            if doc is None:
                logger.warning(f"No ongoing IVR state found for conversation_uuid: {conv_id}")
                return
            ...
            logger.info(f"NCCO generation took {time.time() - start} seconds for conversation_uuid: {conv_id}")
            logger.debug(f"Generated NCCO: {json.dumps(ncco, indent=2)}")

        except Exception as e:
            logger.error(f"Error processing DTMF input message: {e}")
    ...
    async def process_call_event(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            if doc is None:
                logger.warning(f"No ongoing IVR state found for conversation_uuid: {event_data.conversation_uuid}")
                return
            ivr_state = IVRCallStateMongoDoc(**doc)
            ...
            if event_data.status in CallStatus.get_end_call_enums():
                if doc is None:
                    logger.warning(f"No call log found for conversation_uuid: {event_data.conversation_uuid}")
                    return
                ...
                if doc is None:
                    ...
                    logger.info(f"IVR call ended and logs updated for conversation_uuid: {event_data}")
                else:
                ...
                if doc is not None:
                    ...
                    logger.info(f"IVR state updated for conversation_uuid: {event_data.conversation_uuid}")
        ...
        try:
            ...
            if doc is not None:
                ...
                if(datetime.now() - ivr_state.created_at).total_seconds()/60 > int(
                    ...
                    logger.info(f"Ongoing IVR call already exists for phone number: {phone_number}")
                    return {
                        "status_code": 400,
            ...
            logger.debug(f"[START_IVR] Application ID: {os.getenv('VONAGE_APPLICATION_ID')}")
            logger.debug(f"[START_IVR] Private key path: {os.getenv('VONAGE_PRIVATE_KEY_PATH')}")
            ...
            if not latest_fsm:
                logger.error(f"[START_IVR] ✗ FSM not found with ID: {self.latest_fsm_id}")
                return {
                    "status_code": 500,
            ...
            logger.info(f"[START_IVR] ✓ NCCO actions built: {len(ncco_actions)} actions")
            ...
            logger.debug(f"[START_IVR] Call duration limit: {os.getenv('CALL_DURATION_LIMIT')} seconds")
            
            vonage_response = client.voice.create_call({
                "to": [{"type": "phone", "number": phone_number}],
                "from": {"type": "phone", "number": os.getenv("VONAGE_NUMBER")},
                "ncco": ncco_actions,
                "length_timer": int(os.getenv("CALL_DURATION_LIMIT")),
            })
            ...
            logger.info(f"[START_IVR] Conversation UUID: {vonage_response.conversation_uuid}")
            ...
            logger.info(f"IVR call started for phone number: {phone_number}, conversation_uuid: {vonage_response.conversation_uuid}")
            return {
                "status_code": 200,
                "message": f"IVR started for phone number {phone_number}",
            }
        except Exception as e:
            logger.error(f"[START_IVR] ✗ Error starting IVR for phone number {phone_number}: {e}", exc_info=True)
            return {
                "status_code": 500,
    ...
    async def process_message(self, message: QueueMessage):
        """
        Processes a message from the Service Bus.
        Args:
        ...
        try:
            ...
            logger.info(f"[PROCESSOR] Received message of type: {message_type}")
            logger.debug(f"[PROCESSOR] Message payload: {message_body}")

            if message_type == "call_webhook":
                logger.info(f"[PROCESSOR] Routing to process_call_webhook...")
                await self.process_call_webhook(message_body)
                logger.info(f"[PROCESSOR] ✓ process_call_webhook completed")
            elif message_type == "dtmf_input":
                logger.info(f"[PROCESSOR] Routing to process_dtmf_input...")
                await self.process_dtmf_input(message_body)
            elif message_type == "call_event":
                logger.info(f"[PROCESSOR] Routing to process_call_event...")
                await self.process_call_event(message_body)
            else:
                logger.warning(f"[PROCESSOR] ✗ Unknown message type received: {message_type}")
            logger.info(f"[PROCESSOR] ========================================")
        except Exception as e:
    ...
    async def start(self):
        ...
        try:
            while self.running:
                messages = await provider.receive_messages(
                    max_messages=10,
                    wait_time_seconds=5
                )
    ...
    async def stop(self):
        """Stops the CallProcessor."""
        self.running = False
        logger.info("CallProcessor stopped.")
