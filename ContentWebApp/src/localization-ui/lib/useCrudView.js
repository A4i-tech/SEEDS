...
export function useCrudView({
  items, matchFn, getId, emptyValues, onCreate, onUpdate, onDelete, toast, entityLabel,
}) {
  ...
  const rows = useMemo(
    () => items.filter((item) => matchFn(item, q)),
    [items, q, matchFn]
  );
  ...
  const set = (key, value) =>
    setDlg((d) => ({ ...d, values: { ...d.values, [key]: value } }));
  ...
  const save = async () => {
    ...
    try {
      ...
      toast({ message: `${entityLabel} ${dlg.mode === "edit" ? "updated" : "created"}`, tone: "good" });
      setDlg(null);
    } catch (e) {
