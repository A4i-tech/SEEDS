...
export function CrudTable({ columns, rows, getId, onEdit, onDelete, emptyIcon, emptyTitle, emptyMessage }) {
  if (!rows.length) {
      ...
      <tbody>
        {rows.map((row) => (
          <tr key={getId(row)}>
            ...
            <td><RowActions onEdit={() => onEdit(row)} onDelete={() => onDelete(row)} /></td>
          </tr>
        ))}
      </tbody>
    </table>
