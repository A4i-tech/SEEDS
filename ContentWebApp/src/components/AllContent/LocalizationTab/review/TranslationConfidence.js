...
const TranslationConfidence = ({
  ...
  // Real heuristic score from platform/app/services/quality_scorer.py — null
  // until a translation exists, so render as "no data" rather than a fake number.
  const hasScore = typeof qualityScore === "number";
  const confidence = hasScore ? Math.round(qualityScore * 100) : 0;
