...
const teacher = { name: 'Test Teacher', phone_number: '1234567890', role: 'Teacher', is_muted: true, call_status: 'connected', is_raised: false };
const student1 = { name: 'Test Student 1', phone_number: '0987654321', role: 'Student', is_muted: false, call_status: 'connected', is_raised: true };
const student2 = { name: 'Test Student 2', phone_number: '1122334455', role: 'Student', is_muted: true, call_status: 'disconnected', is_raised: false };
...
jest.mock('./context/ConferenceContext', () => ({
    ...jest.requireActual('./context/ConferenceContext'),
...
describe('DetailsPage Component', () => {
    ...
    describe('Rendering', () => {
        test('renders page layout and participant data', () => {
            ...
            expect(screen.getByRole('img', { name: /raised hand/i })).toBeInTheDocument();
        });
    });
    ...
    describe('Participant Management', () => {
        test('handles mute/unmute and reconnect functionality', async () => {
            ...
            await expectApiCall(mockApiService.unmuteParticipant, 'test-conf-123', '1234567890');

            // Test disconnected participant
            mockConferenceContext.userList.push({ ...student2, call_status: 'disconnected' });
            mockConferenceContext.isConfCallRunning = true;
            renderDetailsPage();

            expect(screen.getByText('Reconnect')).toBeInTheDocument();
            fireEvent.click(screen.getByText('Reconnect'));
            await expectApiCall(mockApiService.addParticipant, 'test-conf-123', '1122334455');
        });
    ...
    describe('Audio Control', () => {
        test('handles all music control states', async () => {
            ...
            mockConferenceContext.audioContentState = { status: 'Paused', current_url: 'audio.wav' };
    ...
    describe('Error Handling', () => {
        test('handles API errors gracefully', async () => {
            ...
            mockApiService.startConferenceCall.mockRejectedValue(new Error('API Error'));
            renderDetailsPage();

            fireEvent.click(screen.getByText('Start Call'));
            await waitFor(() => {
                expect(consoleError).toHaveBeenCalledWith('Error starting the call:', expect.any(Error));
            });
