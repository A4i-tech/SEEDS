...
│  │    LEGACY fallback - platform runs conf cmds itself       │   │
│  │    (requiresClientExecution never set by platform)        │   │
│  └──────────────┬───────────────────────────────────────────┘   │
│                 │                          │                     │
...
┌─────────────────────────────────────────────────────────────────┐
│                        BACKEND                                   │
│  meta_controller.py  (FastAPI router, prefix /meta)              │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  POST /meta/voice-command  [require_role]                │   │
│  │  POST /meta/text-command   [require_role]                │   │
│  │  POST /meta/transcribe     [require_role]                │   │
│  │  POST /meta/tts-prompt     [PUBLIC — needed pre-login]   │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  meta_controller.py (orchestrator)                               │
│  ┌──────────────────────────────────────────────────────────┐   │
...
│  │    ↳ /conference/* executed server-side (self)           │   │
│  │  Phase 4: SPEAK   ─► LLM summary → Azure TTS → audio    │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  meta_service.py (engine)                                        │
│  ┌──────────────────────────────────────────────────────────┐   │
...
│  │  • execute_commands()         → aiohttp calls vs self     │   │
│  │    ↳ conference cmds run server-side (no client flag)    │   │
│  │  • transcribe_audio()         → ffmpeg→WAV → Azure STT   │   │
│  │  • formatHistory()            → last 2 turns → prompt ✨ │   │
│  │  • generateSpokenSummary()    → LLM spoken text (no IDs) │   │
│  │  • synthesizeSpeech()         → Azure TTS (SSML→MP3 b64) │   │
│  └──────────────────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────────┘
                           │  internal (same app) + browser
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│           CONFERENCE ROUTES (same platform FastAPI app)         │
│  {API base}/conference (same app)                               │
│  ┌──────────────────────────────────────────────────────────┐   │
...
2. All commands (including `/conference/` paths) are executed via aiohttp against the platform's own REST API (`base_url` + path) with the user's JWT forwarded (15s timeout per step).
3. Conference routes are hosted by the platform itself (`conference_controller.py`) — no proxy, no client delegation.
...
Auth is enforced per-route via FastAPI dependencies in `platform/app/controllers/meta_controller.py`: `require_role("teacher", "content_creator")`.

| Route | Auth |
|---|---|
| `POST /meta/voice-command` | `require_role("teacher", "content_creator")` ✅ |
| `POST /meta/text-command` | `require_role("teacher", "content_creator")` ✅ |
| `POST /meta/transcribe` | `require_role("teacher", "content_creator")` ✅ |
...
| File | Purpose |
|---|---|
| `platform/app/controllers/meta_controller.py` | Router + orchestrator: voice/text-command, transcribe, tts-prompt; 4-phase pipeline, canAutoResolve short-circuit ✨ |
| `platform/app/controllers/class_controller.py` | Class CRUD — resolves phone numbers → Student ObjectIds on POST ✨ |
| `platform/app/controllers/conference_controller.py` | Conference routes (create/start/end/mute/play/...) — hosted in-app, no proxy |
| `platform/app/services/meta_service.py` | Engine: Azure OpenAI LLM, Mongo pre-fetch, STT bias, fuzzy name match, placeholder/forEach/NAVIGATE resolution, history, no-ID TTS ✨ |
| `platform/app/services/tts_service.py` | Azure Speech TTS (SSML build, language→voice map) |
| `docs/STUDENT_ROUTES_ACCESS.md` | Policy doc: students are `school_admin`-only; AI cannot create/edit/delete ✨ |
| `platform/app/platform/security.py` | Auth dependencies — `require_role`, JWT claims (`sub`, `school_id`, `tenant_id`) |
| `platform/app/platform/settings.py` | Settings: `azure_openai_*`, `azure_speech_*`, `tts_*`, `mongo_db_connection_string`, `secret_key` |
...
**Conference delegation flow (LEGACY — inert on platform):**
The Python platform hosts `/conference/*` itself and executes conference commands server-side in Phase 3; it never returns `requiresClientExecution: true`. `executeClientCommands(results)` in `voiceCommandService.js` is kept as a defensive fallback (targets `${REACT_APP_API_BASE_URL}/conference`) but is a no-op against the current backend.
...
| `executeClientCommands(results)` | Legacy fallback — platform never flags PENDING_CLIENT; effectively a no-op |
...
### Conference Calls (platform-hosted)

> These routes are served by the platform itself (`/conference/*`) and executed **server-side** during Phase 3. The webapp also calls them directly (same `REACT_APP_API_BASE_URL` origin) for manual call controls.
...
| `TTS_REGION` / `AZURE_SPEECH_REGION` | Azure Speech resource region (e.g. `centralindia`) — powers both STT and TTS | For STT/TTS |
| `TTS_SUBSCRIPTION_KEY` / `AZURE_SPEECH_KEY` | Azure Speech subscription key | For STT/TTS |
...
| `REACT_APP_CONF_SERVER_BASE_URI` | **Deprecated** — conference routes now live under `REACT_APP_API_BASE_URL` | No |
...
- **CORS:** Conference routes are same-origin with the API now; only the platform's own CORS config for the webapp origin matters.
