...
exports.transferTeacher = async (req, res) => {
    try {
        ...
        if (!teacherId || !targetSchoolId) {
            return res.status(STATUS.BAD_REQUEST).json({ message: "teacherId and targetSchoolId are required" });
        }
        const currentSchoolId = req.schoolId;
        const tenantId = req.tenantId;
        const teacher = await teacherService.transferTeacher(teacherId, currentSchoolId, targetSchoolId, tenantId);
...
exports.register = async (req, res) => {
    ...
    if (!phoneNumber || !password || !schoolId || trimmedName.length === 0 || !role) {
        return res.status(STATUS.BAD_REQUEST).json({ message: "Phone number, password, schoolId, name, and role are required" });
    }
    ...
    try {
        await teacherService.registerTeacher(phoneNumber, password, schoolId, trimmedName, role, tenantId);
...
exports.update = async (req, res) => {
    ...
    if (!name && !phoneNumber && !password) {
        return res.status(STATUS.BAD_REQUEST).json({ message: "At least one field (name, phoneNumber, password) is required" });
    }

    try {
        const teacher = await teacherService.updateTeacher(teacherId, schoolId, { name, phoneNumber, password });
