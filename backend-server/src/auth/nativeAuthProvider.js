...
module.exports = {
  ...
  async login(req, res) {
    ...
    try {
      ...
      const token = jwt.sign(
        {id: tenant._id, email: tenant.email, name: tenant.tenantName},
        secretKey,
        {expiresIn: jwtExpiresIn, issuer: 'tenant', algorithm: 'HS256'}
      );
  ...
  async register(req, res) {
    ...
    try {
      ...
      const tenant = new Tenant({email, password: hashedPassword, tenantName});
      await tenant.save();
